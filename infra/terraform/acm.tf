...
resource "aws_route53_record" "cloudfront_cert_validation" {
  count   = var.enable_domain ? length(aws_acm_certificate.cloudfront.domain_validation_options) : 0
...
resource "aws_route53_record" "alb_cert_validation" {
  count   = var.enable_domain ? length(aws_acm_certificate.alb.domain_validation_options) : 0
  name    = aws_acm_certificate.alb.domain_validation_options[count.index].resource_record_name
  type    = aws_acm_certificate.alb.domain_validation_options[count.index].resource_record_type
  zone_id = aws_route53_zone.primary[0].zone_id
  records = [aws_acm_certificate.alb.domain_validation_options[count.index].resource_record_value]
  ttl     = 60
}
